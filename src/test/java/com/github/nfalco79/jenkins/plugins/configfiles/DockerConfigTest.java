...
import com.github.nfalco79.jenkins.plugins.configfiles.DockerConfig.DockerConfigProvider;
...
public class DockerConfigTest {
    ...
    public void test_load_template() {
        Descriptor<?> descriptor = j.jenkins.getDescriptor(DockerConfig.class);
        Assertions.assertThat(descriptor).isNotNull() //
                .describedAs("Unexpected descriptor class").isInstanceOf(DockerConfigProvider.class);

        DockerConfigProvider provider = (DockerConfigProvider) descriptor;
        Config config = provider.newConfig("testId");
        Assertions.assertThat(config).isNotNull() //
                .describedAs("Unexpected config class").isInstanceOf(DockerConfig.class);
